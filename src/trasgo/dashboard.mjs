...
function loadScientificFixtures() {
  ...
  const bootSeed = fs.readFileSync(path.join(REPO_ROOT, 'src', 'boot.md'), 'utf8');
  ...
  const protocolSeed = [
    fs.readFileSync(path.join(REPO_ROOT, 'src', 'mode-lock.md'), 'utf8'),
    fs.readFileSync(path.join(REPO_ROOT, 'src', 'validate.md'), 'utf8'),
  ].join('\n\n');
